...
const sellerAdsListControllers = async (req, res) => {
  ...
  try {
    ...
    const {
      count,
      error: productError
    } = await supabase.from("walls").select(`*`, {
      count: 'exact',
      head: true
    });
    if (productError) {
    ...
    } = await supabase.from("walls").select('*').eq('created_by', user_id).range(offset, offset + limit - 1);
    if (productListError) {
...
const getProductsController = async (req, res) => {
  ...
  try {
    ...
    const {
      data: productData,
      error: productListError
    } = await supabase.from("walls").select('*').eq('is_available', true).range(offset, offset + limit - 1);
    if (productListError) {
      throw new Error(productListError.message);
    }
...
const singleProductController = async (req, res) => {
  ...
  try {
    const {
      data: product,
      error: productError
    } = await supabase.from("walls").select('*').eq('id', product_id);
    if (productError) {
...
const wallOnRentControllers = async (req, res) => {
  ...
  try {
    const {
      ...
      count: 'exact',
      head: true
    }).eq('created_by', user_id).eq('is_available', false);
    if (countError) {
    ...
    const {
      data: productData,
      error: productError
    } = await supabase.from('walls').select('*').eq('created_by', user_id).eq('is_available', false).range(offset, offset + limit - 1);
...
const singleProductOnRentController = async (req, res) => {
  ...
  try {
    const {
      data: product,
      error: productError
    } = await supabase.from("walls").select('*, users(name, email)').eq('id', product_id);
    if (productError) {
      throw new Error(productError);
    }
    if (product) {
...
const wallOnRentBuyerControllers = async (req, res) => {
  ...
  try {
    const {
      count,
      error: countError
    } = await supabase.from('orders').select('*, walls(*)', {
      count: 'exact',
      head: true
    }).eq('created_by', user_id).eq('payment_status', 'captured');
    if (countError) {
      console.log(countError);
      throw new Error(countError.message);
    }
    ...
    const {
      data: productData,
      error: productError
    } = await supabase.from('orders').select('* , walls(*)').eq('created_by', user_id).eq('payment_status', 'captured').range(offset, offset + limit - 1);
    console.log(productData, productError);
    if (productError) {
      throw new Error(productError.message);
    }
