...
const razopayPaymentVerifyWebhook = async(req, res)=>{
    ...
    if(entity === 'event'){
        ...
        if(event === 'payment.captured'){
            ...
            const {id, status, order_id, method } = hookData?.payload?.payment?.entity;
            ...
            const {data: updateData, error: udpateError } = await supabase.from("orders").update({razorpay_payment_id: id, payment_mode: method, payment_status: status,}).eq('razorpay_order_id', `${order_id}`).select('id');
            ...
            if(updateData != null){
               ...
               const {data: wallsUpdateData, error: wallsUpdateError } = await supabase.from("walls").update({is_available: false}).eq('id', `${product_id}`).select();
